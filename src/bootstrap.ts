...
export const saltboxModule: SaltboxModule = {
  ...
  init: (authStore, services, localeStore, pluginsStore) => {
    ...
    // @ts-ignore
    console.warn(">>> Gateway: toJS function", toJS);
    // @ts-ignore
    console.warn(">>> Gateway: toJS(pluginsStore.plugins)", toJS(pluginsStore.plugins));
    // @ts-ignore
    console.warn(">>> Gateway: Are they same instance?", toJS === window.__MOBX_TOJS__);
    runInAction(() => {
